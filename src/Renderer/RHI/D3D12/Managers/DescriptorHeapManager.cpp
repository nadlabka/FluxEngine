...
void RHI::DescriptorHeapManager::InitHeaps()
{
    ...
    if (auto d3d12device = std::dynamic_pointer_cast<D3D12Device>(device))
    {
        {
    ...
    {
        throw std::runtime_error("Device pointer is incorrect/unintialized. Probably, API has not been initialized");
    }
	
}

std::shared_ptr<RHI::D3D12DescriptorHeap> RHI::DescriptorHeapManager::GetDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE heapType)
{
    switch (heapType)
    {
